...
# Compliant: CMEK, uniform access, all labels. Customer Managed Key.
...
# Non-compliant: no encryption block (trips SC-28). No encryption, but we have labels and public access prevention.
...
# Non-compliant: public access not locked down (trips AC-3). Next step - no public access prevention. but encryption and lables present
...
# Non-compliant: no labels (trips CM-6). Third test.  No lables, but encryption and public access prevention present.
...
# firewall rule to allow SSH traffic from the internet from any IP address.
